...
public class MyStack {
  ...
  synchronized public void push() {
    try {
      while (list.size() > 0) {
        logger.info(Thread.currentThread().getName()+" begin to wait " + System.currentTimeMillis());
        this.wait(); // thread 從 blocked queue 中再回來時，從下面一行開始執行
        logger.info(Thread.currentThread().getName()+" end wait " + System.currentTimeMillis());
